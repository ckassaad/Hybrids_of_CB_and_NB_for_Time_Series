...
def fork_v_structure_generator(T=1000, seed=0, verbose=False, gaussian=True):
    T = T+2
    random.seed(seed)
    ax = uniform_with_gap()/2
    ay = uniform_with_gap()/2
    aw = uniform_with_gap()/2
    axw = uniform_with_gap()/2
    ayw = uniform_with_gap()/2
    awx = uniform_with_gap()/2
    awy = uniform_with_gap()/2
    ...
    for i in range(3, T):
        x[i] = ax * x[i - 1] + awx * w[i - iwx] + bx * epsx[i]
        y[i] = ay * y[i - 1] + awy * w[i - iwy] + by * epsy[i]
...
def diamond_generator(T=1000, seed=0, verbose=False, gaussian=True):
    ...
    ay = uniform_with_gap()/2
    aw = uniform_with_gap()/2
    az = uniform_with_gap()/2
    axy = uniform_with_gap()/2
    axw = uniform_with_gap()/2
    ayz = uniform_with_gap()/2
    awz = uniform_with_gap()/2
    # ayx = - axy
    # awx = - axw
    # azy = - ayz
    # azw = - awz
    ayx = uniform_with_gap()/2
    awx = uniform_with_gap()/2
    azy = uniform_with_gap()/2
    azw = uniform_with_gap()/2
    ...
    for i in range(3, T):
        x[i] = ax * x[i - 1] + ayx * y[i - iyx] + awx * w[i - iwx] + bx * epsx[i]
        y[i] = ay * y[i - 1] + axy * x[i - ixy] + azy * z[i - izy] + by * epsy[i]
        w[i] = aw * w[i - 1] + axw * x[i - ixw] + azw * z[i - izw] + bw * epsw[i]
...
if __name__ == '__main__':
    ...
    param_generator_name = "diamond"  # fork_v_structure diamond
    ...
    for i in range(100):
        print("######################################### iter " + str(i) + " #########################################")
        if param_gaussian:
            directory_name = "feedback_" + param_generator_name + "_g"
        else:
            directory_name = "feedback_" + param_generator_name + "_ng"
        param_data = pd.read_csv("./data/simulated_data/" + directory_name + "/dataset_" + str(i) + ".csv", index_col=0)
        # if param_generator_name == "fork_v_structure":
        #     param_data = fork_v_structure_generator(T=1000, seed=i, gaussian=param_gaussian)
        # elif param_generator_name == "diamond":
        #     a = np.empty([param_nb_obervation, 4])
        #     a[:] = np.nan
        #     param_data = pd.DataFrame(a)
        #     while (param_data.isnull().values.any()) or (np.isinf(param_data.values).any()):
        #         j = i + random.random()
        #         param_data = diamond_generator(T=1000, seed=j, gaussian=param_gaussian)
        # else:
        #     param_data = None
        ...
        if param_generator_name == "fork_v_structure":
            causal_graph_true.add_edge(Edge(GraphNode("V1"), GraphNode("V3"), Endpoint.ARROW, Endpoint.ARROW))
            causal_graph_true.add_edge(Edge(GraphNode("V2"), GraphNode("V3"), Endpoint.ARROW, Endpoint.ARROW))
        elif param_generator_name == "diamond":
            causal_graph_true.add_edge(Edge(GraphNode("V1"), GraphNode("V2"), Endpoint.ARROW, Endpoint.ARROW))
            causal_graph_true.add_edge(Edge(GraphNode("V1"), GraphNode("V3"), Endpoint.ARROW, Endpoint.ARROW))
            causal_graph_true.add_edge(Edge(GraphNode("V2"), GraphNode("V4"), Endpoint.ARROW, Endpoint.ARROW))
            causal_graph_true.add_edge(Edge(GraphNode("V3"), GraphNode("V4"), Endpoint.ARROW, Endpoint.ARROW))
        else:
