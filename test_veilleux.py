...
if __name__ == '__main__':
    ...
    for i in range(0, 2):
        print("######################################### iter " + str(i) + " #########################################")
        param_data = pd.read_csv("./data/veilleux/veilleux_subset_CC0"+ idx_file[i] +".csv", index_col=0, delimiter=";", decimal=',')
        # param_data.columns = param_data.columns.str.replace(' ', '_')
        print(param_data)
        # for col in param_data.columns:
        #     param_data[col] = np.log(param_data[col] + 0.0001)
        ...
        causal_graph_true.add_edge(Edge(GraphNode(param_data.columns[1]), GraphNode(param_data.columns[0]), Endpoint.ARROW, Endpoint.ARROW))
    ...
    print("F1 orientation with desired graph= " + str(np.mean(f1_orientation_list)) + " +- " + str(
        np.var(f1_orientation_list)))
