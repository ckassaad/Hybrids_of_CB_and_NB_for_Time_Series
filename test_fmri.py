...
if __name__ == '__main__':
    ...
    for i in range(1, 29):
        if i == 4:
            continue
        print("######################################### iter " + str(i) + " #########################################")
        param_data = pd.read_csv("./data/fMRI_processed_by_Nauta/returns/timeseries" + str(i) + ".csv", delimiter=',', index_col=False, header=0)
        new_nodes = []
        ...
        param_data.columns = new_nodes

        three_col_format = np.loadtxt("./data/fMRI_processed_by_Nauta/ground_truth/sim"+str(i)+"_gt_processed.csv",
                                      delimiter=',')
